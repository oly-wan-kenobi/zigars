...
Zigar is in **strong overall shape**. The Zig core is production-grade (hexagonal architecture, zero TODO/FIXME markers, comprehensive validation gates), documentation is consistent and current, and CI is multi-platform with 885 tests.

The main caveats are:

1. **v0.2.0 is only half-shipped publicly.** The GitHub release has the 5 Zig tarballs + checksums, but the staged MCPB bundles are not uploaded. The npm shim has not been published to npm.
...
3. **Original skills-package placeholder finding is stale.** Phase 0
   reconciliation found `packages/zigar-skills-npm/package.json`, a CLI,
   README, license, tests, and a concrete `zigar-development` skill. Publish
   readiness still needs package-local validation and any client skill
   validator.
4. **The npm shim is untested at the shim level.** Smoke tests cover the Zig core but never exercise the `bin/zigar-mcp.js` install/launch path.
...
- Distribution (`packages/zigar-mcp-npm`, `packages/zigar-mcpb`, `packages/zigar-skills-npm`, `.github/workflows/`, `dist/`)
- Tests, CI, coverage (`tests/`, `tools/quality/`, `tools/integration/`, `tools/coverage/`, `coverage/`, `.zigar-cache/`)
...
- 41 Zig files in `tools/` (~7.9K LoC) implementing a 15-subcommand dispatcher at [tools/zigar_tools.zig:80-157](tools/zigar_tools.zig).
...
**Build system**
- [build.zig](build.zig) — 299 lines, 20+ build steps. Unit tests embedded alongside source, integration smoke tests (HTTP + stdio), kcov coverage with per-module floors, multi-target release packaging (5 platforms), and continuous validation steps (architecture-guard, public-contract, fake-backend scenarios).
...
- All references use `@zigars/mcp` — **no leftover** `@oly-wan-kenobi/zigar-mcp` or `@zigar/mcp` in any source file.
- Zig 0.16.0 referenced uniformly (README.md, AGENTS.md, backends.md, CI workflows).
- Archive naming convention (`zigar-{arch}-{os}.tar.gz`) consistent across docs and release tooling.
- v0.2.0 / 2026-05-19 dating consistent across CHANGELOG and per-package READMEs.

**Coverage by audience**
- End users: [README.md](README.md), [packages/zigar-mcp-npm/README.md](packages/zigar-mcp-npm/README.md).
...
- Some duplication between README quickstart and `packages/zigar-mcp-npm/README.md`. Tolerable for discoverability but worth designating one as the source of truth.

---

### 3.3 Distribution Packages

#### 3.3.1 `@zigars/mcp` (npm shim) — `packages/zigar-mcp-npm/`

- **Name / version:** `@zigars/mcp@0.2.0`, `mcpName: io.github.oly-wan-kenobi/zigar`, `engines: node >= 18`, `publishConfig: { access: "public" }`. All correct.
- **Entry:** [bin/zigar-mcp.js](packages/zigar-mcp-npm/bin/zigar-mcp.js) → `dist/src/cli` (built by `prepack: npm run build`).
- **Files:** `bin`, `dist/src`, `src`, `README.md`, `LICENSE`, `tsconfig.json`, `dist/package.json`.
- **Verification:** Real SHA-256 checksum verification in [src/checksums.ts](packages/zigar-mcp-npm/src/checksums.ts) + [src/releases.ts](packages/zigar-mcp-npm/src/releases.ts).
...
- **Readiness:** **READY TO PUBLISH.** The v0.2.0 GitHub release has the 5 Zig tarballs the shim needs.

#### 3.3.2 MCPB — `packages/zigar-mcpb/`

- **Role:** Release-only build harness (`private: true`, not for npm).
- **Builder:** [src/build.ts](packages/zigar-mcpb/src/build.ts) shells to `@anthropic-ai/mcpb@2.1.2` (env-overridable), consumes `dist/assets/*.tar.gz`, produces three bundles:
  - `zigar-darwin-universal.mcpb` (x86_64 + aarch64 merged via `llvm-lipo`)
  - `zigar-linux-x64.mcpb` (arm64 users routed to npm shim)
  - `zigar-windows-x64.mcpb`
- **Manifest generation:** Auto-generated `manifest.json` (manifest_version 0.3, name `zigar-mcp`, version 0.2.0, entry `server/zigar`, user config field `workspace`).
- **Staged artifacts in `dist/assets/`:** `zigar-darwin-universal.mcpb` 3.9MB, `zigar-linux-x64.mcpb` 5.7MB, `zigar-windows-x64.mcpb` 2.2MB, `zigar-mcpb-checksums.txt` 270B (3 SHA-256 entries).
- **Readiness:** Build tooling complete, artifacts staged and checksummed. **NOT YET UPLOADED** to the v0.2.0 GitHub release.

#### 3.3.3 `packages/zigar-skills-npm/`

**Phase 0 update (2026-05-27):** The original placeholder finding is stale.
The package now contains `package.json`, `bin/zigar-skills.js`, `README.md`,
`LICENSE`, `test/cli.test.js`, and `skills/zigar-development/SKILL.md`.
...
- Uploads: `dist/assets/*.tar.gz`, `dist/assets/*.mcpb`, `dist/assets/zigar-checksums.txt`, `dist/assets/zigar-mcpb-checksums.txt`.
...
**Correction:** Earlier ICM-recall context claimed all v0.2.0 asset URLs return 404. This is **stale** — the Zig tarballs are live. MCPB bundles are the actual remaining gap.
...
- `zigar-lib-tests`: 824 tests (min 480) ✓
- `zigar-tools-tests`: 57 tests (min 26) ✓
- `zigar-exe-tests`: 2 tests (min 1) ✓
- `zigar-fuzz-tests`: 2 tests (min 2) ✓
...
**Cached evidence in `.zigar-cache/`**
...
1. **Upload MCPB bundles + `zigar-mcpb-checksums.txt` to the v0.2.0 GitHub release.** Either re-run `release.yml` against the existing tag or attach the staged `dist/assets/*.mcpb` files manually. Closes the discrepancy between staged artifacts and what end users can download.
2. **Publish `@zigars/mcp@0.2.0` to npm.** The package is ready; nothing else blocks this.
3. **Validate `packages/zigar-skills-npm/` publish readiness.** The original
...
5. **Add end-to-end smoke tests for the npm shim.** Install from a local tarball into a scratch directory, invoke `bin/zigar-mcp.js`, assert it downloads + verifies + launches. Highest-leverage missing test.
...
|---|---|---|
| "v0.2.0 GitHub release returns 404 for all assets" | ICM recall context | False — 5 Zig tarballs + checksums live since 2026-05-19. MCPB bundles are the actual gap. |
| "Fuzz testing not integrated into CI/CD gates" | Source-audit agent | False — `ci.yml` runs `--fuzz=10K`. |
| "zigar-skills-npm SKILL.md is entirely TODO template, no content beyond boilerplate" | Distribution agent | Stale after Phase 0 reconciliation — package metadata, CLI, README, tests, and a concrete skill now exist. |
...
`packages/zigar-skills-npm` is no longer a placeholder, while missing MCPB
