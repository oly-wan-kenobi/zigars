...
`zigars_capabilities`, `zigars_tool_index`, and `zigars_schema` expose the same
...
and zigars path hints. `zigars_schema` and the `zigars://tools/schema` resource
...
- Source-scan-backed: zigars scanned local files and reports source paths,
...
- External-backend-backed: ZLint, zwanzig, zflame, diff-folded, Samply, Tracy,
  ...
  platform profilers own the backend semantics; zigars reports argv, probes, and
  artifact metadata.
...
Treat `tools/list`, `zigars_schema`, `zigars_tool_index`, and
`zigars://tools/schema` as client-visible API. Tool names, argument fields,
...
`zigars_context_pack.mode` and `zigars_validate_patch.mode` can advertise
...
`readOnlyHint` remains useful for client UI, but zigars' risk fields are more
...
without weakening workspace path policy. `zigars_artifact_index` reads the
workspace-local `.zigars-cache/artifacts/registry.jsonl` registry when present,
scans bounded generated roots such as `.zigars-cache`, `zig-out`, `coverage`, and
...
`zigars_artifact_read` reads one workspace artifact with a caller-specified byte
limit and hash. `zigars_artifact_prune` is preview-first: with `apply=false` it
...
profile contract is `.zigars/profile.json` with `schema_version: 2`; it records
...
`zigars_project_profile_v2` previews or writes that file, and
`zigars_profile_bootstrap`, `zigars_profile_import`, `zigars_profile_diff`,
`zigars_profile_validate`, and `zigars_profile_read` cover generation, import,
comparison, validation, and bounded reads. Writes are preview-first and require
`apply=true`.

`zigars_setup_guidance`, `zigars_profile_guidance`, and
`zigars_backend_guidance` return detected facts, questions, and unknowns for
...
`zigars_env_pack` returns a reproducible environment pack with configured Zig,
...
`zigars_env_export` writes the pack under `.zigars-cache/env/` only when
`apply=true` and registers artifact provenance. `zig_toolchain_pin` previews or
writes `.zigars/toolchain.json`; `zig_toolchain_pin_check` compares that pin with
...
ZVM tools are inert by default. `zigars_zvm_probe` runs bounded read-only ZVM
commands, while `zigars_zvm_install_plan` and `zigars_zvm_switch_plan` return
...
Backend setup tools follow the same contract. `zigars_backend_install_plan`
returns setup commands and verification steps from the backend catalog without
running them. `zigars_backend_verify` runs bounded probes for configured paths.
`zigars_dev_env_generate` previews or writes pinned mise, asdf, Nix,
devcontainer, or GitHub Actions setup artifacts. `zigars_backend_conformance`
returns conformance scenarios and evidence paths, and
`zigars_backend_evidence_pack` turns an existing conformance report into a compact
...
installing dependencies or broadening setup claims. `zigars_adoption_pack` is
...
`zigars_client_config_generate` previews deterministic client configuration for
...
`zigars_smoke_plan` returns a client, transport, platform, timeout, and backend
...
`zigars_conformance_report` ingests inline or workspace conformance JSON and
...
`zigars_result_shape` describes the public compact, standard, and deep response
...
where applicable. `zigars_output_budget_plan` returns planning budgets for a
...
`zigars_metrics_v2` reports in-memory runtime counters for the current server
...
observed command durations. `zigars_backend_health_history`,
`zigars_zls_timeline`, and `zigars_tool_latency` expose those sections
individually.

Observability data resets when the zigars process restarts. Backend history
records probes observed through zigars helper paths; it does not watch external
backend state independently. Command durations are observed for commands routed
through shared zigars helpers, and tool latency is MCP validation plus handler
dispatch time, not client/network serialization time. Use `zigars_doctor
...
interaction without weakening the workspace guard. `zigars_job_start` and
`zigars_run_stream` run allow-listed Zig commands (`build`, `build-test`,
`test`, `check`, and `fmt-check`) without a shell, retain a bounded job record,
and return output tails, event cursors, and a job id. `zigars_job_status`,
`zigars_job_result`, `zigars_run_events`, `zigars_job_cancel`, and
`zigars_cancel_status` read or update that retained state. Jobs and events are
...
`zigars_resource_subscribe` and `zigars_resource_unsubscribe`. They are
process-local subscription records for client coordination; they do not start a
filesystem watcher. `zigars_roots_sync`, `zigars_workspace_map`, and
`zigars_workspace_select` expose client root guidance while file tools continue
to resolve paths inside the configured zigars workspace.

`completion/complete` returns values for prompt names, resource URIs, workflow
names, allow-listed command names, and client names. `zigars_agent_guide_v2`,
`zigars_client_guide`, and `zigars_prompt_pack` summarize shipped workflows for
...
the text and `structuredContent` fallback. `zigars_patch_session_apply` may issue
...
path. `zigars_failure_fusion` accepts `summarize=true` and may issue
...
`zigars_validation_plan` returns a risk-aware validation plan without executing
commands. It reports changed-file facts, command phases, read-only tool phases,
skipped phases, unknowns, and a stop condition. `zigars_validation_run` executes
...
preview-first: `.zigars-cache/validation/history.jsonl` is appended only when
...
`zigars_validation_history`, `zig_test_flake_history`, and
...
`zigars_session_snapshot` and `zigars_handoff_pack` package current goal,
changed files, validation state, profile state, workspace metadata, recommended
next steps, and limitations for client handoff. `zigars_decision_record`
previews or appends structured decision records only with `apply=true`;
`zigars_project_notes` and `zigars_project_memory` read those records and expose
built-in zigars policies such as generated-path and apply-gate rules.

`zigars_capability_match` ranks registered zigars tools for a goal, error, or
diff using manifest descriptions, keywords, risk, and confidence metadata.
`zigars_tool_sequence_plan` returns an ordered tool sequence with execution-risk
...
preimage evidence. `zigars_patch_session_create` records current file identities
and generated/vendor policy for the requested paths. `zigars_patch_session_preview`
returns per-file diffs plus `expected_preimages`; `zigars_patch_session_apply`
...
`.zigars-cache/patch-sessions/` and append a bounded JSONL history record.
`zigars_patch_session_revert` restores only files whose current hash still matches
the recorded session output, and `zigars_patch_session_validate` delegates to the
...
`zig_generated_file_trace` classifies one path, `zigars_edit_policy_check`
checks files or patch text before broad edits, and `zigars_generated_route`
...
`zig_api_baseline_init` previews or writes a `.zigars-cache` baseline artifact
...
`zigars_docs_drift_check`, `zigars_release_claim_check`, and
`zigars_tool_index_check` are fast, read-only guards for public docs and
...
be installed for default zigars operation.
...
`zigars_doctor` accepts optional `probe_backends` and `timeout_ms` arguments. Use
...
server process and surfaced through `zigars_workspace_info` and `zigars_metrics`.

Docs tools are intentionally split by source, and each source reports
provenance/completeness. `zig_builtin_*` uses curated zigars data, records the
...
`.zigars-cache/profile/diff-<n>.folded` unless an explicit workspace-local
