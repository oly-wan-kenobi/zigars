...
- `coverage/` is generated by `zig build coverage` and is not part of the
  published package.
- Keep `src/tool_manifest.zig` as the single authority for tool names, schemas,
  grouping, discovery keywords, risk flags, command-plan metadata, and handler
  references when adding, renaming, or removing tools.
