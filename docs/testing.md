...
zigars uses layered tests so transport behavior, tool registration, and backend
...
| Integration | Public behavior from the built `zigars` binary over HTTP or stdio. Integration fixtures assert MCP schema, transport, tool-call, artifact, resource, prompt, and report contracts; they do not assert internal module paths or handler implementation details. |
...
`zig build test` runs four compiled test binaries: `zigars-lib-tests`,
`zigars-exe-tests`, `zigars-tools-tests`, and `zigars-fuzz-tests`. All four have
...
`zig build smoke` starts zigars over HTTP and checks `initialize`,
`tools/list`, `zigars_schema`, `zigars_doctor`, and representative tool calls
...
`zig build stdio-fixtures` starts zigars over stdio and uses a temporary
...
workflow. It starts zigars over stdio with real backend paths, probes them through
`zigars_doctor`, runs one representative tool for each backend family, and
verifies zflame/diff-folded SVG artifacts. Use `ZIGARS_ZLS_PATH`,
`ZIGARS_ZLINT_PATH`, `ZIGARS_ZWANZIG_PATH`, `ZIGARS_ZFLAME_PATH`, and
`ZIGARS_DIFF_FOLDED_PATH` when the binaries are not on `PATH`. The check writes
`report.json`, `summary.md`, `stdout.jsonl`, and `stderr.log` under
`.zigars-cache/backend-conformance/` by default, and the workflow uploads those
files as `zigars-backend-conformance`.
...
complete `coverage/` directory as the `zigars-coverage` artifact. A
...
`zigars-fuzz-tests` uses the repository's custom runner at
