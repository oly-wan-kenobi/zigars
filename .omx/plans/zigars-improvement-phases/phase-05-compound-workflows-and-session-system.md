...
- All session paths stay under `.zigars-cache/` or another workspace-safe cache
  root.
...
2. Store sessions under a cache-local layout.
   - Preferred: `.zigars-cache/sessions/<kind>/<id>.jsonl`.
...
7. Build `zig_watch`.
   ...
   - Use a `zigars://.../events` style URI for event reads.
