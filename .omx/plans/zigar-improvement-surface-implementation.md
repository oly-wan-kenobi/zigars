# Zigar Improvement Surface Implementation Plan
...
Phase handoff documents live under `.omx/plans/zigar-improvement-phases/`.
Each phase document is self-contained enough for a fresh implementation session:

- Phase 0: `.omx/plans/zigar-improvement-phases/phase-00-baseline-reconciliation.md`
- Phase 1: `.omx/plans/zigar-improvement-phases/phase-01-mcp-contract-foundations.md`
- Phase 2: `.omx/plans/zigar-improvement-phases/phase-02-dependency-lifecycle.md`
- Phase 3: `.omx/plans/zigar-improvement-phases/phase-03-architecture-neutral-agent-ergonomics.md`
- Phase 4: `.omx/plans/zigar-improvement-phases/phase-04-zig-developer-pain-analyzers.md`
- Phase 5: `.omx/plans/zigar-improvement-phases/phase-05-compound-workflows-and-session-system.md`
- Phase 6: `.omx/plans/zigar-improvement-phases/phase-06-protocol-feature-pilots.md`
- Phase 7: `.omx/plans/zigar-improvement-phases/phase-07-docs-skills-release-adoption.md`

## Guiding Decisions

1. Keep zigar deterministic. Do not add AI code-generation behavior inside the server.
...
11. Zigar's hexagonal architecture rules remain internal to zigar. Do not expose zigar-specific layer policy as a default public Zig tool; any public architecture-policy surface must be explicit and opt-in.
...
- Persistent sessions survive process restart where claimed, reject stale preimages, and expose inspectable artifacts under `.zigar-cache/`.
- Zigar's internal architecture guard remains a quality gate, not a public `zig_*` tool. Public tools may use import graphs and project-local evidence, but must not enforce zigar's folder/layer policy on other projects by default.
...
2. Re-run release-hygiene checks from `CLAUDE_ANALYSIS.md` because some findings are stale: `packages/zigar-skills-npm/package.json` and tests now exist, but release/npm/MCPB/coverage status may still need verification.
...
- `packages/zigar-skills-npm/`
- `tools/quality/architecture_guard.zig`

Validation:

- `zig build docs-check json-check`
- `npm test` in `packages/zigar-skills-npm` if that package is touched
...
4. Add prompt/skill guidance after the tools exist.
   - Update `zigar_prompt_pack` or equivalent prompts.
   - Keep `packages/zigar-skills-npm/skills/zigar-development/SKILL.md` concise and route to tools rather than duplicating tool docs.
...
- `packages/zigar-skills-npm/skills/zigar-development/SKILL.md`

Acceptance:

- `zig_import_cycles` returns empty cycles for current zigar source unless a fixture introduces one.
- Planner/reviewer tools label confidence and limitations.
- No new public tool implies that arbitrary Zig projects should follow zigar's internal hexagonal layout.
...
Goal: add Zig-specific analysis where zigar can be better than generic MCP peers.
...
1. Introduce a shared session envelope.
   ...
   - `validation`
   - `schema_version`
2. Store sessions under `.zigar-cache/sessions/<kind>/<id>.jsonl` or an equivalent cache-local layout.
...
7. Build `zig_watch`.
   - Async job plus `zigar://run/events` style cursored resource is canonical.
...
Goal: use elicitation and sampling where they improve existing workflows without making zigar dependent on client support.

Steps:

1. Rename advisory tools.
   - `zigar_setup_elicit` to `zigar_setup_guidance`
   - `zigar_profile_elicit` to `zigar_profile_guidance`
   - `zigar_backend_elicit` to `zigar_backend_guidance`
...
3. Keep package READMEs aligned.
   - `packages/zigar-mcp-npm/README.md`
   - `packages/zigar-skills-npm/README.md`
...
- Public architecture-policy checking: defer unless there is demand for an explicit opt-in project profile. Zigar's internal hexagonal guard is not a default public Zig policy.
...
| Public tools accidentally enforce zigar's internal hexagonal layout | Keep zigar's architecture guard internal unless a future explicit opt-in project profile is designed. |
