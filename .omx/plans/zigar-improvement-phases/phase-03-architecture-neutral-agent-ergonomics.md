...
- Public tools must be architecture-neutral. Do not add `zig_architecture_layer`
  or default enforcement of zigar's internal hexagonal layout.
...
- `zig_import_cycles` returns empty cycles for current zigar source unless a
  fixture introduces one.
- Planner/reviewer tools label confidence and limitations.
- No new public tool implies arbitrary Zig projects should follow zigar's
