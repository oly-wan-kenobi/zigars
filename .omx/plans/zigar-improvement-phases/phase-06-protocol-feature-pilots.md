...
making zigar dependent on supporting clients.
...
- Sampling must use the host LLM only for summarization or classification of
  provided evidence, not code generation inside zigar.
...
- Environment advisory tools currently use `_elicit` naming in manifest/docs:
  `zigar_setup_elicit`, `zigar_profile_elicit`, and `zigar_backend_elicit`.
