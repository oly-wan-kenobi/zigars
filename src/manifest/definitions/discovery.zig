...
/// Return a compact zigar tool/capability index with search keywords, including fmt, formatter, formatting, and zig fmt.
...
/// Return a compact searchable zigar tool index with aliases for fmt, formatter, formatting, zig fmt, docs, ZLS, lint, and profiling.
...
/// Return zigar's compact tool catalog and schema-discovery hints.
...
/// Diagnose common zigar MCP configuration, workspace, backend, and transport problems.
...
/// Return zigar process counters and backend health.
...
/// Preview the exact argv/cwd/timeout for a deterministic zigar command workflow; use zig_tool_plan for non-command-backed tools.
...
/// Return manifest-derived planning support for any registered zigar tool, including exact commands, dynamic backends, ZLS requests, and pure analysis tools.
