...
/// Return bounded ZLS status transition history for the current zigar process.
...
/// Return per-tool handler latency and error counters for the current zigar process.
...
/// Summarize zigar path policy, backend identities, risk metadata, dependency hashes, and optional clean-tree evidence.
...
/// Describe compact, standard, and deep zigar result-shape contracts with explicit omission metadata.
