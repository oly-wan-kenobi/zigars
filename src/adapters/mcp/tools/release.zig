...
test "release docs adapters exercise public wrappers through ports" {
    ...
    const builtin_list = try zigBuiltinList(allocator, context, null);
    ...
    try expectNoBuiltinEnv(&toolchain);
    const builtin_list_json = try zigBuiltinListJson(allocator, context, null);
    ...
    try expectNoBuiltinEnv(&toolchain);
    const builtin_doc = try zigBuiltinDoc(allocator, context, .{ .object = builtin_args });
    ...
    try expectNoBuiltinEnv(&toolchain);
    const builtin_doc_json = try zigBuiltinDocJson(allocator, context, .{ .object = builtin_json_args });
    ...
    const std_search = try zigStdSearch(allocator, context, .{ .object = std_search_args });
    ...
    try expectStdScan(&toolchain, &scanner, "release_docs.std_search", std_source);
    const std_search_json = try zigStdSearchJson(allocator, context, .{ .object = std_search_json_args });
    ...
    const std_item = try zigStdItem(allocator, context, .{ .object = std_item_args });
    ...
    try expectStdScan(&toolchain, &scanner, "release_docs.std_item", std_source);
    const std_item_json = try zigStdItemJson(allocator, context, .{ .object = std_item_json_args });
    ...
    try expectStdScan(&toolchain, &scanner, "release_docs.std_item", std_source);
    const signature = try zigStdSignature(allocator, context, .{ .object = signature_args });
    ...
    const langref = try zigLangRefSearch(allocator, context, .{ .object = langref_args });
    ...
    try expectLangref(&toolchain, &scanner, langref_probe, langref_html);
    const langref_json = try zigLangRefSearchJson(allocator, context, .{ .object = langref_json_args });
    ...
    try expectLangref(&toolchain, &scanner, langref_probe, langref_html);
    const langref_item = try zigLangrefItem(allocator, context, .{ .object = langref_item_args });
    ...
    const docs_index = try zigDocsIndexBuild(allocator, context, .{ .object = index_args });
    ...
    const docs_query = try zigDocsQuery(allocator, context, .{ .object = docs_query_args });
    ...
    const project_query = try zigProjectDocsQuery(allocator, context, .{ .object = project_query_args });
    ...
    const autodoc = try zigAutodocIngest(allocator, context, .{ .object = autodoc_args });
    ...
    const examples = try zigDocExampleCheck(allocator, context, .{ .object = example_args });
    ...
    const readme = try zigReadmeCommandCheck(allocator, context, .{ .object = readme_args });
    defer mcp_result.deinitToolResult(allocator, readme);
    try expectStructuredKind(readme, "zig_readme_command_check");

    const missing_builtin = try zigBuiltinDoc(allocator, context, null);
