...
pub fn zigarAdoptionPack(a: *App, result_allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigarClientConfigGenerate(a: *App, result_allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    const client = argString(args, "client") orelse "generic";
    const transport = argString(args, "transport") orelse transportName(a);
    ...
    try obj.put(scratch, "server_argv", try common.argvValue(scratch, argv));
...
pub fn zigarSmokePlan(a: *App, result_allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    const client = argString(args, "client") orelse "generic";
    const transport = argString(args, "transport") orelse transportName(a);
    const backend = argString(args, "backend") orelse "all";
...
pub fn zigarConformanceReport(a: *App, result_allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    const backend = argString(args, "backend") orelse "all";
...
fn catalogSnapshotValue(allocator: std.mem.Allocator, a: *App, backend: []const u8) !std.json.Value {
    ...
    try obj.put(allocator, "platform", .{ .string = hostPlatformName() });
...
fn smokePlanValue(allocator: std.mem.Allocator, a: *App, client: []const u8, transport: []const u8, backend: []const u8, platform: []const u8, timeout_ms: i64) !std.json.ObjectMap {
    ...
    try obj.put(allocator, "platform", .{ .string = if (std.mem.eql(u8, platform, "current") or std.mem.eql(u8, platform, "native")) hostPlatformName() else platform });
...
fn evidenceReadError(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value, err: anyerror) mcp.tools.ToolError!mcp.tools.ToolResult {
    const path = argString(args, "input") orelse default_conformance_input;
...
fn configContent(allocator: std.mem.Allocator, a: *App, client: []const u8, transport: []const u8, kind: []const u8, server_path: []const u8) ![]u8 {
    ...
    if (std.mem.eql(u8, kind, "mcp-json") or std.mem.eql(u8, kind, "claude-json") or std.mem.eql(u8, kind, "gemini-json")) {
        ...
        try server.put(allocator, "args", try common.argvValue(allocator, argv[1..]));
...
fn workspacePathExists(a: *App, path: []const u8) bool {
    ...
    defer a.workspace.allocator.free(resolved);
    std.Io.Dir.cwd().access(a.io, resolved, .{}) catch return false;
    return true;
}
