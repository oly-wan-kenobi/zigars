...
pub fn zigDebugPlan(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    if (argBool(args, "probe_backend", false)) {
        const probe = common.probeBackend(a, scratch, "lldb", &.{ lldb_path, "--version" }, @min(common.toolTimeout(a, args), 5000));
...
pub fn zigLldbBacktrace(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    if (builtin.os.tag == .windows) return unsupportedBackendResult(allocator, "lldb", "backtrace", "LLDB backtrace capture is not supported by zigar on Windows.");
...
pub fn zigCoreInspect(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    if (builtin.os.tag == .windows) return unsupportedBackendResult(allocator, "lldb", "core_inspect", "Core dump inspection is not supported by zigar on Windows.");
...
pub fn zigDebugFrameSummary(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigSanitizerFusion(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigPanicTraceAnalyze(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigCrashReproPlan(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigHeaptrackRun(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const command_argv = common.splitToolArgs(allocator, command_text) catch |err| return common.splitToolArgsErrorResult(allocator, "zig_heaptrack_run", "command", command_text, err);
    ...
    if (builtin.os.tag != .linux) return unsupportedBackendResult(allocator, "heaptrack", "run", "heaptrack capture is supported by zigar only on Linux.");
...
pub fn zigHeaptrackSummary(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigValgrindMemcheck(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const command_argv = common.splitToolArgs(allocator, command_text) catch |err| return common.splitToolArgsErrorResult(allocator, "zig_valgrind_memcheck", "command", command_text, err);
    defer freeArgv(allocator, command_argv);
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
    ...
    if (builtin.os.tag != .linux) return unsupportedBackendResult(allocator, "valgrind", "memcheck", "Valgrind memcheck is supported by zigar only on Linux.");
    return applyGatedBackendCommand(a, allocator, scratch, args, .{
...
pub fn zigCallgrindReport(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const command_argv = common.splitToolArgs(allocator, command_text) catch |err| return common.splitToolArgsErrorResult(allocator, "zig_callgrind_report", "command", command_text, err);
    defer freeArgv(allocator, command_argv);
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
    ...
    if (builtin.os.tag != .linux) return unsupportedBackendResult(allocator, "valgrind", "callgrind", "Valgrind callgrind is supported by zigar only on Linux.");
    return applyGatedBackendCommand(a, allocator, scratch, args, .{
...
pub fn zigFuzzPlan(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
...
pub fn zigAflRun(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const command_argv = common.splitToolArgs(allocator, command_text) catch |err| return common.splitToolArgsErrorResult(allocator, "zig_afl_run", "command", command_text, err);
    defer freeArgv(allocator, command_argv);
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
    ...
    if (builtin.os.tag == .windows) return unsupportedBackendResult(allocator, "afl-fuzz", "run", "AFL++ execution is not supported by zigar on Windows.");
    return applyGatedBackendCommand(a, allocator, scratch, args, .{
...
pub fn zigLibfuzzerRun(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const argv = common.splitToolArgs(allocator, command_text) catch |err| return common.splitToolArgsErrorResult(allocator, "zig_libfuzzer_run", "command", command_text, err);
...
pub fn zigFuzzCrashMinimize(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigFuzzCorpusSummary(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigBinarySize(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigBinarySizeDiff(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigObjdumpSummary(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigDwarfdumpCheck(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    return binaryBackendTool(a, allocator, args, .{
...
pub fn zigSymbolize(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
...
pub fn zigQemuTest(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const command_argv = common.splitToolArgs(allocator, command_text) catch |err| return common.splitToolArgsErrorResult(allocator, "zig_qemu_test", "command", command_text, err);
    defer freeArgv(allocator, command_argv);
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
    ...
    if (builtin.os.tag == .windows) return unsupportedBackendResult(allocator, "qemu", "test", "QEMU execution is not supported by zigar on Windows.");
    return applyGatedBackendCommand(a, allocator, scratch, args, .{
...
pub fn zigCrossSmoke(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    return crossPlan(a, allocator, args, "zig_cross_smoke", "Cross-target smoke matrix");
}

pub fn zigTargetRuntimePlan(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    return crossPlan(a, allocator, args, "zig_target_runtime_plan", "Target runtime plan");
}

pub fn zigEmbeddedDetect(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const value = embeddedDetectValue(arena.allocator(), a, @intCast(@max(1, argInt(args, "limit", 80)))) catch |err| return common.toolErrorFromError(allocator, .{
...
pub fn zigMicrozigPlan(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
...
pub fn zigBoardProfile(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
...
pub fn zigFlashPlan(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) mcp.tools.ToolError!mcp.tools.ToolResult {
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
    ...
    if (argBool(args, "probe_backend", false)) {
        const probe = common.probeBackend(a, scratch, flash_tool, &.{ flash_tool, "--help" }, @min(common.toolTimeout(a, args), 5000));
...
fn lldbCapture(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value, tool_name: []const u8, binary: []const u8, core: ?[]const u8, command_expr: []const u8, basis: []const u8) mcp.tools.ToolError!mcp.tools.ToolResult {
    const lldb_path = argString(args, "lldb_path") orelse "lldb";
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
    ...
    const probe = common.probeBackend(a, scratch, "lldb", &.{ lldb_path, "--version" }, @min(common.toolTimeout(a, args), 5000));
    if (!probe.ok) return common.backendUnavailableResult(allocator, "lldb", "debug_capture", lldb_path, probe.status, "Install LLDB separately or pass lldb_path to an existing executable; zigar never installs debugger tools.");
    const result = command.run(allocator, a.io, a.workspace.root, argv, common.toolTimeout(a, args)) catch |err| return common.backendErrorResult(allocator, "lldb", "debug_capture", err, "Run the shown LLDB argv directly to inspect debugger-specific failures.");
...
fn applyGatedBackendCommand(a: *App, result_allocator: std.mem.Allocator, scratch: std.mem.Allocator, args: ?std.json.Value, spec: BackendCommandSpec) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const probe = common.probeBackend(a, scratch, spec.backend_name, spec.probe_argv, @min(common.toolTimeout(a, args), 5000));
    if (!probe.ok) return common.backendUnavailableResult(result_allocator, spec.backend_name, spec.operation, spec.configured_path, probe.status, "Install or configure the optional backend outside zigar, then retry with the path argument; zigar never installs external tools.");
    const run = command.run(result_allocator, a.io, a.workspace.root, spec.argv, common.toolTimeout(a, args)) catch |err| return common.backendErrorResult(result_allocator, spec.backend_name, spec.operation, err, "Run the shown backend argv directly to inspect backend-specific failures.");
    ...
    try obj.put(scratch, "command_argv", try common.argvValue(scratch, spec.argv));
    try obj.put(scratch, "command_result", try common.commandResultValue(scratch, spec.basis, spec.argv, a.workspace.root, common.toolTimeout(a, args), run));
    ...
    const bytes = try json_result.serializeAlloc(scratch, .{ .object = obj });
...
fn applyGatedUserCommand(a: *App, result_allocator: std.mem.Allocator, scratch: std.mem.Allocator, args: ?std.json.Value, tool_name: []const u8, argv: []const []const u8, output: []const u8, artifact_kind: []const u8, notes: []const u8) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const run = command.run(result_allocator, a.io, a.workspace.root, argv, common.toolTimeout(a, args)) catch |err| {
        const value = common.commandErrorValue(scratch, "project diagnostic command", argv, a.workspace.root, common.toolTimeout(a, args), err) catch return error.OutOfMemory;
    ...
    try obj.put(scratch, "command_argv", try common.argvValue(scratch, argv));
    try obj.put(scratch, "command_result", try common.commandResultValue(scratch, "project diagnostic command", argv, a.workspace.root, common.toolTimeout(a, args), run));
...
fn binaryBackendTool(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value, spec: BinaryBackendSpec) mcp.tools.ToolError!mcp.tools.ToolResult {
...
fn binaryApplyGatedCommand(a: *App, result_allocator: std.mem.Allocator, scratch: std.mem.Allocator, args: ?std.json.Value, tool_name: []const u8, backend_name: []const u8, operation: []const u8, configured_path: []const u8, argv: []const []const u8, path: []const u8, basis: []const u8) mcp.tools.ToolError!mcp.tools.ToolResult {
    ...
    const probe = common.probeBackend(a, scratch, backend_name, &.{ configured_path, "--version" }, @min(common.toolTimeout(a, args), 5000));
    if (!probe.ok) return common.backendUnavailableResult(result_allocator, backend_name, operation, configured_path, probe.status, "Install the LLVM binary tools separately or pass the per-call backend path; zigar never installs them.");
    const run = command.run(result_allocator, a.io, a.workspace.root, argv, common.toolTimeout(a, args)) catch |err| return common.backendErrorResult(result_allocator, backend_name, operation, err, "Run the shown backend argv directly to inspect binary-tool-specific failures.");
    defer run.deinit(result_allocator);
    ...
    try obj.put(scratch, "command_argv", try common.argvValue(scratch, argv));
    try obj.put(scratch, "command_result", try common.commandResultValue(scratch, basis, argv, a.workspace.root, common.toolTimeout(a, args), run));
    try putBinaryInfo(scratch, &obj, info);
...
fn previewResult(a: *App, result_allocator: std.mem.Allocator, scratch: std.mem.Allocator, tool_name: []const u8, backend: []const u8, configured_path: []const u8, argv: []const []const u8, basis: []const u8, target: ?[]const u8, output: ?[]const u8) mcp.tools.ToolError!mcp.tools.ToolResult {
    var obj = try baseValue(scratch, a, tool_name, basis, "medium", &.{
...
fn memorySummary(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value, tool_name: []const u8, input: EvidenceInput, backend: []const u8) mcp.tools.ToolError!mcp.tools.ToolResult {
    _ = args;
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
...
fn callgrindSummary(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value, input: EvidenceInput) mcp.tools.ToolError!mcp.tools.ToolResult {
    _ = args;
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
...
fn crossPlan(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value, tool_name: []const u8, basis: []const u8) mcp.tools.ToolError!mcp.tools.ToolResult {
    var arena = std.heap.ArenaAllocator.init(allocator);
    defer arena.deinit();
    const scratch = arena.allocator();
    var obj = try baseValue(scratch, a, tool_name, basis, "medium", &.{
...
fn corpusSummaryValue(allocator: std.mem.Allocator, a: *App, path: []const u8, limit: usize) !std.json.Value {
    ...
    var dir = try std.Io.Dir.openDirAbsolute(a.io, abs, .{ .iterate = true });
...
fn embeddedDetectValue(allocator: std.mem.Allocator, a: *App, limit: usize) !std.json.Value {
    ...
    while (seen < limit) {
        ...
        try item.put(allocator, "line", .{ .integer = @intCast(common.lineNumberLocal(bytes, firstEmbeddedSignalIndex(bytes))) });
...
fn baseValue(allocator: std.mem.Allocator, a: *App, kind: []const u8, evidence_basis: []const u8, confidence: []const u8, limitations: []const []const u8) !std.json.ObjectMap {
    ...
    try obj.put(allocator, "platform", try platformValue(allocator));
...
fn platformValue(allocator: std.mem.Allocator) !std.json.Value {
    var obj = std.json.ObjectMap.empty;
    try obj.put(allocator, "os", .{ .string = @tagName(builtin.os.tag) });
    try obj.put(allocator, "arch", .{ .string = @tagName(builtin.cpu.arch) });
    return .{ .object = obj };
}
...
fn writeEvidenceArtifact(a: *App, allocator: std.mem.Allocator, path: []const u8, value: std.json.Value, producer: []const u8, artifact_kind: []const u8, argv: []const []const u8, backend: []const u8, target: []const u8, notes: []const u8) !void {
    const bytes = try json_result.serializeAlloc(allocator, value);
...
test "memory, callgrind, target, and embedded helpers expose stable fields" {
    ...
    const findings = try memoryFindingsValue(allocator, "definitely lost: 32 bytes in 1 blocks\nERROR SUMMARY: 2 errors\nallocations: 7 allocations\n");
    try std.testing.expectEqual(@as(i64, 32), findings.object.get("definitely_lost_bytes").?.integer);
    try std.testing.expectEqual(@as(i64, 2), findings.object.get("error_count").?.integer);
    const events = try callgrindEventsValue(allocator, "events: Ir Dr\nfn=parse\n10 42\n");
    try std.testing.expectEqualStrings("Ir", events.array.items[0].string);
    const hotspots = try callgrindHotspotsValue(allocator, "events: Ir\nfn=parse\n10 42\n", 4);
    try std.testing.expectEqualStrings("parse", hotspots.array.items[0].object.get("function").?.string);
    const target = try targetPlanValue(allocator, "wasm32-freestanding", null);
    try std.testing.expectEqualStrings("wasm", target.object.get("runtime_class").?.string);
    try std.testing.expectEqualStrings("qemu-aarch64", qemuDefaultForTarget("aarch64-linux-gnu"));
    try std.testing.expectEqualStrings("microzig", embeddedSignal("boards/rp2040.zig", "const microzig = 1;"));
    try std.testing.expect(firstEmbeddedSignalIndex("before flash after") > 0);
    try std.testing.expectEqualStrings("thumb-freestanding-eabi", boardTarget("rp2040"));
    try std.testing.expect(isCortexBoard("stm32"));
    const flash = try flashCommand(allocator, "probe-rs", "rp2040", "firmware.elf");
    try std.testing.expect(std.mem.indexOf(u8, flash, "download") != null);
}
...
test "diagnostic classifiers cover common runtime variants" {
    ...
    try std.testing.expect(containsAny("hello sanitizer", &.{ "none", "sanitizer" }));
    try std.testing.expect(!containsAny("hello", &.{ "asan", "ubsan" }));
    try std.testing.expectEqualStrings("bare_metal", runtimeClass("thumb-freestanding-eabi"));
    try std.testing.expectEqualStrings("host_or_emulated_os", runtimeClass("x86_64-linux-gnu"));
    try std.testing.expectEqualStrings("host_os_specific", runtimeClass("x86_64-macos"));
    try std.testing.expectEqualStrings("unknown", runtimeClass("mystery-target"));
    try std.testing.expectEqualStrings("qemu-arm", qemuDefaultForTarget("arm-linux-gnueabihf"));
    try std.testing.expectEqualStrings("qemu-riscv64", qemuDefaultForTarget("riscv64-linux-gnu"));
    try std.testing.expectEqualStrings("qemu-riscv32", qemuDefaultForTarget("riscv32-freestanding"));
    try std.testing.expectEqualStrings("qemu-system", qemuDefaultForTarget("wasm32-freestanding"));
    try std.testing.expectEqualStrings("xtensa-freestanding-none", boardTarget("esp32"));
    try std.testing.expectEqualStrings("project_defined", boardTarget("custom"));
    try std.testing.expect(!isCortexBoard("esp32"));
}
