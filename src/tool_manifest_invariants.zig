...
test "manifest side-effect metadata stays aligned with planning policy" {
    for (tool_manifest.entries) |entry| {
        ...
        if (risk.writes_source) {
            try std.testing.expect(risk.writes_require_apply);
            try std.testing.expect(risk.preview_by_default);
            try std.testing.expect(!tool_manifest.readOnlyHintFor(entry.meta));
            try std.testing.expect(!tool_manifest.idempotentHintFor(entry.meta));
        }
...
test "public api diff text snapshots are not advertised as input paths" {
    const entry = tool_manifest.findEntry("zig_public_api_diff") orelse return error.MissingTool;
...
test "tool group keyword metadata covers each group once" {
    const fields = @typeInfo(tool_manifest.ToolGroup).@"enum".fields;
    try std.testing.expectEqual(fields.len, tool_manifest.group_specs.len);
    inline for (fields) |field| {
        var count: usize = 0;
        for (tool_manifest.group_specs) |spec| {
            if (std.mem.eql(u8, tool_manifest.groupName(spec.group), field.name)) {
                count += 1;
...
fn runsFromRuntimeArguments(entry: tool_manifest.ToolEntry) bool {
