...
pub fn zigarsReleaseClaimCheck(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) !Result {
    ...
    const value = try driftResultValue(allocator, .{
        .kind = "zigars_release_claim_check",
        .mode = mode,
        .ok = ok,
        .checks = checks,
...
/// Executes the zigars tool index check workflow and returns an allocator-owned structured result.
pub fn zigarsToolIndexCheck(a: *App, allocator: std.mem.Allocator, args: ?std.json.Value) !Result {
    const mode = parseModeArg(allocator, "zigars_tool_index_check", args) catch |err| return modeError(allocator, "zigars_tool_index_check", args, err);
    ...
    // Drift passes only when every registered tool is mentioned AND the
    // generator header is present: the header guards against an index that was
    // hand-edited rather than regenerated from the manifest.
    const ok = missing.items.len == 0 and std.mem.indexOf(u8, bytes, "Generated by zigars-tools generate-tool-index") != null;
    var checks = std.json.Array.init(allocator);
    var checks_owned = true;
    defer if (checks_owned) checks.deinit();
...
/// Returns the required public-contract marker substrings expected in a given
...
/// Returns true only if every check object reports ok=true; used to fold
/// per-file results into the overall drift verdict.
...
/// Reads a workspace-relative doc file (capped at 8 MiB) through the sandboxed
/// workspace; the path is resolved under the workspace boundary before any read.
/// Returns workspace-allocator-owned bytes the caller must free. The `allocator`
/// parameter is unused (kept for call-site uniformity); allocation uses the
/// workspace allocator.
...
/// Parses the optional "mode" argument into a Mode, defaulting to standard when
/// absent and returning error.InvalidMode for an unrecognized value (callers map
...
/// Converts a parseModeArg failure into a structured argument_error result;
/// any error other than InvalidMode is treated as an allocation failure and
/// propagated as error.OutOfMemory.
...
/// Attaches the shared mode/result_shape/omitted_sections metadata block so
/// every drift result advertises its verbosity mode and which sections the mode
/// intentionally dropped (so absence is never mistaken for a passing check).
...
/// Human-readable list of accepted modes, reused in argument-error messages.
...
const TestManifest = struct {
    ...
    /// Returns the fixture entry whose name matches, or null when absent.
...
const DriftHarness = struct {
    ...
    /// Asserts every fixture port saw exactly its expected calls.
...
/// Builds a `{ "mode": <mode> }` JSON arguments object for tests; the borrowed
/// `mode` string must outlive the returned value.
...
/// Queues a successful workspace read for `path` returning `bytes` (matching the
/// fixed max_bytes/provenance the drift workflow uses).
...
/// Queues a failing workspace read for `path` so drift checks exercise the
...
test "docs drift check reports compact metadata missing docs and manifest tool needles" {
    ...
    try expectDriftRead(&harness.workspace, "docs/tool-index.generated.md",
        \\Generated by zigars-tools generate-tool-index
        \\`zig_build`
    );
    ...
    const result = try zigarsDocsDriftCheck(&app, arena.allocator(), try argsWithMode(arena.allocator(), "compact"));
...
test "release claim and tool index checks cover invalid modes read failures and missing tools" {
    ...
    var app = harness.app(arena.allocator());
    const invalid = try zigarsReleaseClaimCheck(&app, arena.allocator(), try argsWithMode(arena.allocator(), "wide"));
    ...
    const claims = try zigarsReleaseClaimCheck(&app, arena.allocator(), null);
    ...
    const index = try zigarsToolIndexCheck(&app, arena.allocator(), try argsWithMode(arena.allocator(), "deep"));
    ...
    const failed_index = try zigarsToolIndexCheck(&app, arena.allocator(), null);
