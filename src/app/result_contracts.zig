...
test "result shape contract is typed data independent of renderer payloads" {
    const contract = describeResultShape(.{ .mode = .deep });

    try std.testing.expectEqualStrings("zigar_result_shape", contract.kind);
    try std.testing.expect(contract.ok);
    try std.testing.expectEqual(OutputMode.deep, contract.mode);
    try std.testing.expectEqual(OutputMode.standard, contract.default_mode);
    try std.testing.expectEqual(@as(usize, 3), contract.supported_modes.len);
    try std.testing.expectEqual(OutputMode.deep, contract.selected_mode_metadata.mode);
...
test "output budget plan clamps typed request budgets" {
    ...
    try std.testing.expectEqual(OutputMode.compact, plan.mode);
    try std.testing.expectEqual(@as(i64, min_token_budget), plan.effective_token_budget);
...
test "mode parsing returns a typed app error without raw argument payloads" {
    const parsed = parseOutputMode("verbose");
