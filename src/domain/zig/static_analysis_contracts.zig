...
pub const contracts = [_]Contract{
    ...
    .{ .tool = "zig_import_cycles", .analysis_kind = "architecture_neutral_import_cycle_scc", .tier = .advisory_orientation, .confidence = .medium, .classification = .advisory, .source_coverage = workspace_text_coverage, .limitations = &.{"Cycle detection follows workspace-relative string-literal .zig imports and does not apply project-specific architecture policy."}, .verify_with = &.{ "zig_import_graph_json", "zig build test", "ZLS references" } },
