...
pub const contracts = [_]Contract{
    ...
    .{ .tool = "zig_import_cycles", .analysis_kind = "architecture_neutral_import_cycle_scc", .tier = .advisory_orientation, .confidence = .medium, .classification = .advisory, .source_coverage = workspace_text_coverage, .limitations = &.{"Cycle detection follows workspace-relative string-literal .zig imports and does not apply project-specific architecture policy."}, .verify_with = &.{ "zig_import_graph", "zig build test", "ZLS references" } },
    ...
    .{ .tool = "zig_test_name_resolve", .analysis_kind = "parser_backed_test_name_resolution", .tier = .parser_backed, .confidence = .high, .classification = .advisory, .source_coverage = semantic_index_coverage, .limitations = semantic_index_limits, .verify_with = &.{ "zig ast-check", "zig test --test-filter", "zig build test" } },
    .{ .tool = "zig_test_fixture_inventory", .analysis_kind = "parser_backed_test_fixture_inventory", .tier = .parser_backed, .confidence = .high, .classification = .advisory, .source_coverage = semantic_index_coverage, .limitations = semantic_index_limits, .verify_with = &.{ "zig ast-check", "ZLS references", "zig build test" } },
    .{ .tool = "zig_safety_site_catalog", .analysis_kind = "safety_keyword_site_catalog", .tier = .advisory_orientation, .confidence = .medium, .classification = .advisory, .source_coverage = workspace_text_coverage, .limitations = &.{"Safety-site findings are review prompts and do not prove memory or runtime unsafety."}, .verify_with = &.{ "code review", "zig build test", "configured linters" } },
    .{ .tool = "zig_test_for_symbol", .analysis_kind = "symbol_to_test_candidate_map", .tier = .advisory_orientation, .confidence = .medium, .classification = .advisory, .source_coverage = workspace_text_coverage, .limitations = test_scan_limits, .verify_with = &.{ "zig test --test-filter", "zig build test" } },
    .{ .tool = "zig_module_surface", .analysis_kind = "parser_backed_module_surface", .tier = .parser_backed, .confidence = .high, .classification = .advisory, .source_coverage = semantic_index_coverage, .limitations = semantic_index_limits, .verify_with = &.{ "zig ast-check", "ZLS references", "zig build test" } },
    .{ .tool = "zig_symbol_dossier", .analysis_kind = "symbol_scoped_static_dossier", .tier = .advisory_orientation, .confidence = .medium, .classification = .advisory, .source_coverage = semantic_refs_coverage, .limitations = semantic_refs_limits, .verify_with = &.{ "zig_semantic_decl", "zig_semantic_refs", "zig build test" } },
    .{ .tool = "zig_change_risk_audit", .analysis_kind = "architecture_neutral_change_risk_audit", .tier = .advisory_orientation, .confidence = .medium, .classification = .advisory, .source_coverage = semantic_impact_coverage, .limitations = semantic_impact_limits, .verify_with = &.{ "zig_changed_files_plan", "zig build test", "project CI" } },
    .{ .tool = "zig_insertion_sites", .analysis_kind = "project_local_insertion_site_ranking", .tier = .advisory_orientation, .confidence = .medium, .classification = .orientation_only, .source_coverage = workspace_text_coverage, .limitations = &.{"Recommendations are non-authoritative and architecture-neutral; inspect nearby modules before editing."}, .verify_with = &.{ "code review", "zig_module_surface", "zig build test" } },
    .{ .tool = "zig_io_migration_scan", .analysis_kind = "zig_016_io_migration_catalog", .tier = .advisory_orientation, .confidence = .medium, .classification = .advisory, .source_coverage = workspace_text_coverage, .limitations = &.{"Catalogs likely migration sites only; buffer ownership and concrete reader/writer types require compiler-backed verification."}, .verify_with = &.{ "zig fmt --check .", "zig build test", "zig build -Doptimize=ReleaseSafe" } },
    .{ .tool = "zig_leak_triage", .analysis_kind = "gpa_leak_stderr_grouping", .tier = .advisory_orientation, .confidence = .medium, .classification = .advisory, .source_coverage = "Caller-supplied or workspace-read GPA leak stderr text; repeated traces are grouped by first Zig stack frame when present.", .limitations = &.{"Does not execute symbolizers or inspect binaries; optimized or stripped traces can omit allocation sites."}, .verify_with = &.{ "rerun failing test with GPA leak detection", "debug build stack traces" } },
    .{ .tool = "zig_comptime_diagnose", .analysis_kind = "parser_only_comptime_diagnosis", .tier = .advisory_orientation, .confidence = .medium, .classification = .advisory, .source_coverage = "Caller-supplied source snippet, workspace file, or compiler diagnostic text mined for comptime-known/runtime-known failure clues.", .limitations = &.{"Does not execute compiler probes or evaluate comptime code; compiler diagnostic locations remain authoritative."}, .verify_with = &.{ "zig build test", "zig check", "compiler diagnostic location" } },
    .{ .tool = "zig_memory_layout", .analysis_kind = "layout_sensitive_declaration_catalog_with_optional_compiler_measurements", .tier = .compiler_backed, .confidence = .medium, .classification = .advisory, .source_coverage = "Default mode scans workspace source; measure=true runs standalone compiler-backed zig build-obj probes without project imports, build.zig execution, or target binary execution.", .limitations = &.{ "compiler-backed mode skips project imports, @embedFile, and explicit comptime logic unless opted in.", "Standalone probes cannot measure declarations that require project module resolution." }, .verify_with = &.{ "zig_abi_layout_diff measure=true", "compiler @sizeOf/@alignOf probes", "targeted tests" } },
    .{ .tool = "zig_unsafe_operations_audit", .analysis_kind = "unsafe_boundary_operation_catalog", .tier = .advisory_orientation, .confidence = .medium, .classification = .advisory, .source_coverage = workspace_text_coverage, .limitations = &.{"Review catalog only; it does not prove the matched operation is unsafe or incorrect."}, .verify_with = &.{ "zig_safety_site_catalog", "code review", "zig build test" } },
    .{ .tool = "zig_abi_layout_diff", .analysis_kind = "abi_layout_target_comparison_with_optional_compiler_measurements", .tier = .compiler_backed, .confidence = .medium, .classification = .advisory, .source_coverage = "Default mode plans packed/extern source probes; measure=true compares standalone compiler-backed layout measurements across target triples.", .limitations = &.{ "compiler-backed mode runs direct zig build-obj -fno-emit-bin probes only.", "It does not execute build.zig or target binaries and refuses project imports by default." }, .verify_with = &.{ "zig_memory_layout measure=true", "target-specific ABI tests", "code review at FFI boundaries" } },
...
test "every dispatched static-analysis tool name resolves to a contract" {
    ...
    const dispatched_tool_names = [_][]const u8{
        "zig_import_graph",            "zig_import_cycles",
        "zig_ast_imports",             "zig_decl_summary",
        "zig_ast_decl_summary",        "zig_allocations",
        "zig_error_sets",              "zig_public_api",
        "zig_dead_decl_candidates",    "zig_build_graph",
        "zig_build_targets",           "zig_build_options",
        "zig_file_owner",              "zig_import_resolve",
        "zig_test_discover",           "zig_ast_tests",
        "zig_test_name_resolve",       "zig_test_fixture_inventory",
        "zig_safety_site_catalog",     "zig_test_for_symbol",
        "zig_module_surface",          "zig_symbol_dossier",
        "zig_change_risk_audit",       "zig_insertion_sites",
        "zig_io_migration_scan",       "zig_leak_triage",
        "zig_comptime_diagnose",       "zig_memory_layout",
        "zig_unsafe_operations_audit", "zig_abi_layout_diff",
        ...
        "zig_test_map",                "zig_test_select",
        "zig_public_api_diff",         "zig_semantic_index_build",
        "zig_semantic_index_status",   "zig_semantic_index_refresh",
        "zig_semantic_query",          "zig_semantic_refs",
        "zig_semantic_decl",           "zig_semantic_callers",
        "zig_static_fusion",           "zig_code_index_export",
        "zig_scip_export",             "zig_zlint",
        "zig_zlint_sarif",             "zig_zlint_rules",
        "zig_zlint_fix",               "zig_lint_compare",
        "zig_lint_profile",            "zig_lint_gate",
        "zig_lint_fix_plan",           "zig_lint_baseline",
        "zig_lint_suppressions",       "zig_lint_trend",
        "zig_impact_semantic",         "zig_test_select_semantic",
        "zig_lint",                    "zig_lint_sarif",
