...
pub const contracts = [_]Contract{
    ...
    .{ .tool = "zig_memory_layout", .analysis_kind = "layout_sensitive_declaration_catalog", .tier = .advisory_orientation, .confidence = .medium, .classification = .advisory, .source_coverage = workspace_text_coverage, .limitations = &.{"Actual size, alignment, and offsets require target-specific compiler probes."}, .verify_with = &.{ "zig_abi_layout_diff", "compiler @sizeOf/@alignOf probes", "targeted tests" } },
    ...
    .{ .tool = "zig_abi_layout_diff", .analysis_kind = "abi_layout_probe_plan", .tier = .advisory_orientation, .confidence = .low, .classification = .orientation_only, .source_coverage = workspace_text_coverage, .limitations = &.{"Phase 4 implementation plans probes only; it does not execute compiler commands or compare target outputs."}, .verify_with = &.{ "future compiler-probe backend", "zig_memory_layout", "target-specific ABI tests" } },
