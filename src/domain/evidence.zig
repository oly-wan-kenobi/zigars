...
test "finding summary counts severities" {
    ...
    try findings.append(try findingValue(allocator, .zlint, "rule", "error", "src/main.zig", 3, 1, "bad", .high));
    try findings.append(try findingValue(allocator, .zwanzig, "style", "warning", "src/main.zig", 4, 1, "warn", .high));
    const summary = try summaryValue(allocator, findings);
