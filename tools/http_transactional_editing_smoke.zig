...
pub fn run(allocator: std.mem.Allocator, io: Io, port: u16, expected: JsonValue, scenario_count: *usize) !void {
    ...
    try assertToolPaths(allocator, io, port, 119, "zigar_patch_session_preview", "{\"edits\":\"[{\\\"file\\\":\\\"tests/fixtures/static-analysis/legacy-usingnamespace.zig\\\",\\\"content\\\":\\\"const std = @import(\\\\\\\"std\\\\\\\");\\\\n\\\\nusingnamespace std;\\\\n// fixture preview\\\\n\\\"}]\"}", expected, "patch_session_preview_paths", scenario_count);
    ...
    try assertToolPaths(allocator, io, port, 128, "zig_move_decl", "{\"source_file\":\"tests/fixtures/static-analysis/tricky.zig\",\"target_file\":\"tests/fixtures/static-analysis/legacy-usingnamespace.zig\",\"name\":\"LocalErrors\",\"apply\":false}", expected, "move_decl_paths", scenario_count);
    try assertToolPaths(allocator, io, port, 129, "zig_extract_decl", "{\"file\":\"tests/fixtures/static-analysis/tricky.zig\",\"target_file\":\"tests/fixtures/static-analysis/legacy-usingnamespace.zig\",\"start_line\":25,\"end_line\":27,\"apply\":false}", expected, "extract_decl_paths", scenario_count);
