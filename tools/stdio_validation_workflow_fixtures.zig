pub fn run(client: anytype) !void {
    ...
    const validation_plan = try client.callTool("zigar_validation_plan", "{\"mode\":\"quick\",\"changed_files\":\"notes.txt\"}");
    defer client.allocator.free(validation_plan);
    try client.expectPathString(validation_plan, "kind", "zigar_validation_plan");
    try client.expectPathString(validation_plan, "mode", "quick");

    const validation_run = try client.callTool("zigar_validation_run", "{\"mode\":\"quick\",\"changed_files\":\"notes.txt\",\"apply\":false}");
    defer client.allocator.free(validation_run);
    try client.expectPathString(validation_run, "kind", "zigar_validation_run");
    ...
    const validation_history = try client.callTool("zigar_validation_history", "{}");
    defer client.allocator.free(validation_history);
    try client.expectPathString(validation_history, "kind", "zigar_validation_history");
    ...
    const session_snapshot = try client.callTool("zigar_session_snapshot", "{\"goal\":\"handoff validation\",\"changed_files\":\"src/main.zig\"}");
    defer client.allocator.free(session_snapshot);
    try client.expectPathString(session_snapshot, "kind", "zigar_session_snapshot");
    try client.expectPathString(session_snapshot, "goal", "handoff validation");

    const handoff_pack = try client.callTool("zigar_handoff_pack", "{\"goal\":\"handoff validation\",\"changed_files\":\"src/main.zig\"}");
    defer client.allocator.free(handoff_pack);
    try client.expectPathString(handoff_pack, "kind", "zigar_handoff_pack");
    try client.expectPathJson(handoff_pack, "portable", .{ .bool = true });

    const decision = try client.callTool("zigar_decision_record", "{\"title\":\"Use apply gates\",\"decision\":\"Writes require apply=true\",\"apply\":false}");
    defer client.allocator.free(decision);
    try client.expectPathString(decision, "kind", "zigar_decision_record");
    try client.expectPathJson(decision, "applied", .{ .bool = false });

    const notes = try client.callTool("zigar_project_notes", "{\"content\":\"{\\\"category\\\":\\\"architecture\\\",\\\"title\\\":\\\"Apply gates\\\",\\\"decision\\\":\\\"Writes need apply=true\\\"}\\n\",\"query\":\"apply\"}");
    defer client.allocator.free(notes);
    try client.expectPathString(notes, "kind", "zigar_project_notes");
    try client.expectPathJson(notes, "note_count", .{ .integer = 1 });

    const memory = try client.callTool("zigar_project_memory", "{\"content\":\"{\\\"category\\\":\\\"architecture\\\",\\\"title\\\":\\\"Apply gates\\\",\\\"decision\\\":\\\"Writes need apply=true\\\"}\\n\",\"query\":\"apply\"}");
    defer client.allocator.free(memory);
    try client.expectPathString(memory, "kind", "zigar_project_memory");
    try client.expectPathJson(memory, "note_count", .{ .integer = 1 });

    const capability_match = try client.callTool("zigar_capability_match", "{\"goal\":\"plan validation for changed tests\",\"limit\":3}");
    defer client.allocator.free(capability_match);
    try client.expectPathString(capability_match, "kind", "zigar_capability_match");

    const sequence_plan = try client.callTool("zigar_tool_sequence_plan", "{\"goal\":\"fix failing tests\",\"changed_files\":\"src/main.zig\"}");
