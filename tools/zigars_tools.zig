...
const coverage = @import("coverage/coverage.zig");
...
const http_phase6_smoke = @import("integration/http/http_phase6_smoke.zig");
const http_runtime_ux_smoke = @import("integration/http/http_runtime_ux_smoke.zig");
const http_smoke = @import("integration/http/http_smoke.zig");
const http_diagnostics_smoke = @import("integration/http/http_diagnostics_smoke.zig");
const http_transactional_editing_smoke = @import("integration/http/http_transactional_editing_smoke.zig");
const http_validation_workflow_smoke = @import("integration/http/http_validation_workflow_smoke.zig");
const hex_arch_inventory = @import("quality/hex_arch_inventory.zig");
const json_query = @import("common/json_query.zig");
const json_util = @import("common/json_util.zig");
const mcp_contracts = @import("release/mcp_contracts.zig");
const public_claims = @import("release/public_claims.zig");
const public_contracts = @import("release/public_contracts.zig");
const release_docs = @import("release/release_docs.zig");
const release_checks = @import("release/release_checks.zig");
const release_rules = @import("release/release_rules.zig");
const release_targets = @import("release/release_targets.zig");
const smoke_support = @import("integration/smoke_support.zig");
const stdio_adoption_fixtures = @import("integration/stdio/stdio_adoption_fixtures.zig");
const stdio_environment_fixtures = @import("integration/stdio/stdio_environment_fixtures.zig");
const stdio_fixtures = @import("integration/stdio/stdio_fixtures.zig");
const stdio_runtime_ux_fixtures = @import("integration/stdio/stdio_runtime_ux_fixtures.zig");
const stdio_transactional_editing_fixtures = @import("integration/stdio/stdio_transactional_editing_fixtures.zig");
const stdio_validation_workflow_fixtures = @import("integration/stdio/stdio_validation_workflow_fixtures.zig");
const tool_index = @import("quality/tool_index.zig");
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len < 2) {
        try usage(io);
        return failUsage(io, "zigars-tools", "", "missing command", .{});
    }
    ...
    if (std.mem.eql(u8, cmd, "version")) {
        ...
        try usage(io);
        return failUsage(io, "zigars-tools", "", "unknown command `{s}`", .{cmd});
    }
}
