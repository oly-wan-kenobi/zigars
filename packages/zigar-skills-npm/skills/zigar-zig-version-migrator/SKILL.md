---
name: zigar-zig-version-migrator
...
1. Establish the target and current versions with `zig_version`, `zig_env`,
   `zigar_env_pack`, `zig_toolchain_resolve`, `zig_toolchain_pin_check`,
   `zigar_profile_read`, or `zigar_profile_validate`.
2. Check editor and backend compatibility with `zig_zls_match_check`,
   `zigar_backend_guidance`, and `zigar_backend_verify` when backend behavior is
   part of the task.
