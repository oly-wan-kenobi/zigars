...
leaving deterministic checks to zigar MCP tools.

## Workflow

1. Orient on the workspace and change goal with `zigar_context_pack`,
   `zigar_next_action`, `zig_changed_files_plan`, or
   `zigar_validation_plan` when available.
...
3. Check edit boundaries with `zig_generated_file_trace`,
   `zigar_edit_policy_check`, or `zigar_generated_route` before touching
...
5. Use preview-first write paths. For zigar mutating tools, require
