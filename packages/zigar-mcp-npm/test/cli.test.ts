...
test("spawns installed zigar with normalized args and keeps stdout reserved", async () => {
  let spawnCall: {
    executable: string;
  ...
  const result = await captureRun(["--workspace", "/tmp/example"], {
    installZigar: async (target) => {
      assert.equal(target.archiveName, "zigar-x86_64-linux-musl.tar.gz");
      return "/cache/zigar";
    },
...
test("reports unsupported targets on stderr only", async () => {
  ...
  assert.equal(result.stdout, "");
  assert.match(result.stderr, /Unsupported zigar host target: sunos\/x64/);
});
...
test("prints help to stderr only", async () => {
  ...
  assert.equal(result.stdout, "");
  assert.match(result.stderr, /Usage: zigar-mcp/);
});
...
test("bin entrypoint runs the CLI wrapper", () => {
  const result = spawnSync(process.execPath, [
    path.join(process.cwd(), "bin", "zigar-mcp.js"),
    "--help",
  ], {
    encoding: "utf8",
  });
