...
export function userCacheRoot(env: NodeJS.ProcessEnv = process.env, platform: NodeJS.Platform | string = process.platform): string {
  ...
  if (platform === "win32") {
    return path.join(env.LOCALAPPDATA ?? env.APPDATA ?? path.join(os.homedir(), "AppData", "Local"), "zigar-mcp");
  }
  if (platform === "darwin") {
    return path.join(os.homedir(), "Library", "Caches", "zigar-mcp");
  }
  return path.join(env.XDG_CACHE_HOME ?? path.join(os.homedir(), ".cache"), "zigar-mcp");
}
...
export async function installZigar(target: Readonly<HostTarget>, options: InstallOptions = {}): Promise<string> {
