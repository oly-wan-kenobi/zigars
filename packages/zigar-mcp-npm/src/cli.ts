...
import { normalizeZigarArgs } from "./args";
import { installZigar, type InstallOptions } from "./install";
...
export interface RunOptions extends InstallOptions {
  ...
  installZigar?: (target: Readonly<HostTarget>, options: InstallOptions) => Promise<string>;
  spawn?: SpawnImpl;
...
export async function run(argv: unknown, options: RunOptions = {}): Promise<number> {
  ...
  try {
    ...
    const zigarArgs = normalizeZigarArgs(args);
    const executablePath = await (options.installZigar ?? installZigar)(target, {
