...
exports.installZigar = installZigar;
...
class InstallError extends Error {
    ...
    constructor(message, options = {}) {
        ...
        this.code = options.code ?? "ERR_ZIGAR_INSTALL";
...
function userCacheRoot(env = process.env, platform = process.platform) {
    if (env.ZIGAR_MCP_CACHE_DIR) {
        return env.ZIGAR_MCP_CACHE_DIR;
    }
    if (platform === "win32") {
        return node_path_1.default.join(env.LOCALAPPDATA ?? env.APPDATA ?? node_path_1.default.join(node_os_1.default.homedir(), "AppData", "Local"), "zigar-mcp");
    }
    if (platform === "darwin") {
        return node_path_1.default.join(node_os_1.default.homedir(), "Library", "Caches", "zigar-mcp");
    }
    return node_path_1.default.join(env.XDG_CACHE_HOME ?? node_path_1.default.join(node_os_1.default.homedir(), ".cache"), "zigar-mcp");
}
...
function extractArchive(archivePath, destination, spawnSyncImpl = childProcess.spawnSync) {
    ...
    if (result.error) {
        throw new InstallError(`Failed to extract zigar archive: ${result.error.message}`, {
            code: "ERR_ZIGAR_EXTRACT",
            cause: result.error,
        });
    }
    if (result.status !== 0) {
        ...
        throw new InstallError(`Failed to extract zigar archive${detail}`, {
            code: "ERR_ZIGAR_EXTRACT",
        });
    }
}
async function installZigar(target, options = {}) {
    ...
    if (!options.fetch && typeof globalThis.fetch !== "function") {
        throw new InstallError("This Node.js runtime does not provide fetch", {
            code: "ERR_ZIGAR_FETCH_UNAVAILABLE",
        });
    }
    ...
    try {
        ...
        if (!extractedExecutable) {
            throw new InstallError(`Archive did not contain ${target.executableName}`, {
                code: "ERR_ZIGAR_ARCHIVE_CONTENTS",
            });
        }
