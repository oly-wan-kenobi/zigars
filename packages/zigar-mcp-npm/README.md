...
`@zigars/mcp` is the npm entry point for `zigar`, a deterministic local MCP
server for Zig development.

It is a small TypeScript shim: it downloads the matching `zigar` binary from
...
server, and downloaded binary are still named `zigar`.
...
zigar gives MCP-capable agents a structured Zig workbench. It is not an AI code
...
|---|---|
| Discovery and health | `zigar_capabilities`, `zigar_tool_index`, `zigar_schema`, `zigar_backend_catalog`, `zigar_doctor`, `zigar_workspace_info` |
| Agent workflow | `zigar_context_pack`, `zigar_next_action`, `zigar_agent_guide`, `zigar_validate_patch`, `zigar_validation_plan` |
| Core Zig | `zig_version`, `zig_env`, `zig_targets`, `zig_build`, `zig_test`, `zig_check`, `zig_translate_c` |
| Formatting and transactional edits | `zig_format`, `zig_format_check`, `zig_patch_preview`, `zigar_patch_session_preview`, `zigar_patch_session_apply` |
...
| Artifact registry | `zigar_artifact_index`, `zigar_artifact_read`, `zigar_artifact_prune` |
| Trust and release drift | `zigar_trust_report`, `zigar_command_provenance`, `zigar_clean_tree_gate`, `zigar_docs_drift_check`, `zigar_release_claim_check` |
| Public adoption | `zigar_client_config_generate`, `zigar_adoption_pack`, `zigar_smoke_plan`, `zigar_conformance_report` |
...
| Node.js | 18 | Supported runtime for npm/npx and installed `zigar-mcp` commands. |
| npm/npx | bundled with Node | Use `npx -y @zigars/mcp@0.2.0 ...` when Bun is not available. |
| Yarn | 4 with `dlx` | Use `yarn dlx -p @zigars/mcp@0.2.0 zigar-mcp ...`. |
| pnpm | 10 or newer | Use `pnpm dlx --package @zigars/mcp@0.2.0 zigar-mcp ...`. |
...
|---|---|
| Linux x64 | `zigar-x86_64-linux-musl.tar.gz` |
| Linux arm64 | `zigar-aarch64-linux-musl.tar.gz` |
| macOS x64 | `zigar-x86_64-macos.tar.gz` |
| macOS arm64 | `zigar-aarch64-macos.tar.gz` |
| Windows x64 | `zigar-x86_64-windows.tar.gz` |
...
The shim accepts zigar server arguments and forwards them to the downloaded
...
| Argument | Purpose |
|---|---|
| `--workspace <path>` | Workspace root zigar is allowed to serve. Prefer an absolute path. |
...
| `--cache-dir <path>` | zigar server cache directory. Different from the npm shim cache. |
...
MCPB bundles include the zigar server binary directly and ask for the workspace
...
https://github.com/oly-wan-kenobi/zigar/releases/download/v0.2.0/<host-archive>
```
...
| 3 | Download `zigar-checksums.txt`. |
| 4 | Download the selected archive. |
| 5 | Verify the archive SHA-256. |
| 6 | Extract `zigar` or `zigar.exe` into the user cache. |
...
| Platform | Cache root |
|---|---|
| Linux | `$XDG_CACHE_HOME/zigar-mcp` or `~/.cache/zigar-mcp` |
| macOS | `~/Library/Caches/zigar-mcp` |
| Windows | `%LOCALAPPDATA%\zigar-mcp` or `%APPDATA%\zigar-mcp` |

Override the shim cache:

```sh
ZIGAR_MCP_CACHE_DIR=/absolute/cache/dir \
  bunx --bun @zigars/mcp@0.2.0 --workspace /absolute/path/to/zig/project
```
...
```text
Unsupported zigar host target: <platform>/<arch>
```

Use Linux x64/arm64, macOS x64/arm64, Windows x64, or build zigar from source.
The shim does not silently fall back to another platform archive.
...
Do not bypass checksum verification. Confirm `zigar-checksums.txt` and the
...
Use `probe_backends:true` when you want zigar to execute short backend probes.
...
| `bin/` | `zigar-mcp` executable wrapper. |
...
npm exec --yes --package ./zigars-mcp-0.2.0.tgz -- \
  zigar-mcp --help
bunx --bun --package ./zigars-mcp-0.2.0.tgz zigar-mcp --help
npm exec --yes --package ./zigars-mcp-0.2.0.tgz -- \
