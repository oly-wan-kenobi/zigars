---
name: zigars-release-claim-auditor
...
2. Gather release evidence with `zig_release_plan`, `zig_semver_suggest`,
   ...
   `zigars_clean_tree_gate`, `zigars_trust_report`, and
   `zigars_command_provenance` when available.
3. Check API and docs claims with `zig_public_api_diff`, `zig_api_check`,
   `zig_api_diff_baseline`, `zig_api_docs_diff`,
   `zigars_docs_drift_check`, and `zigars_release_claim_check`.
