...
preserve raw CI authority while using zigars to normalize, group, and reproduce
failures locally.
...
3. Use parser confidence, raw-reference hashes, and limitations from zigars
   results when ranking failures.
4. Build a local reproduction plan with `zig_ci_repro_plan`, changed-file hints,
   `zigars_validation_plan`, and focused build/test commands.
