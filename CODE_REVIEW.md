...
### M1 — Reentrant dispatch → unbounded recursion + serial-invariant break
**VERIFIED** · `src/adapters/mcp/server/protocol_client.zig:166`, reachable via `src/adapters/mcp/tools/transactional_editing.zig:193`
...
### M2 — npm shim cache poisoning: cached binary run without checksum re-verification
**VERIFIED** · `packages/@zigars/mcp/src/install.ts:94-100` (stored-but-unused hash at `:225`)
...
### M6 — zon manifest injection via unescaped fields
**VERIFIED** · `src/domain/zig/zon_dependencies.zig:193`
...
- **L1** Unguarded re-`initialize` resets `state` to `.initializing` and overwrites client info/capabilities mid-session (`src/adapters/mcp/server.zig:495`); the spec says once-only. Bounded impact. Fix: reject when `state != .uninitialized`.
...
- **L6** Checksum comparison is not constant-time (`packages/@zigars/mcp/src/checksums.ts:61`); low practical impact (hash of attacker-supplied data).
